...
function mountWidthSlider(steps, chance) {
  ...
  const props = { min: n1, max: n2, value: x, label: chance.word() }
  ...
  const wrapper = mount(WidthSlider, { propsData: props });
...
function expectEvent(steps, wrapper, eventName, expectedPayload, i) {
  steps.push(`expect component to emit an <code>input</code> event with a payload of <code>${xrand(expectedPayload)}</code>`)
...
describe('WidthSlider', () => {
  test_(101, 'Text field initial value', (steps, chance) => {
    ...
    expectFieldValue(steps, wrapper, 'input[type="number"]', String(props.value / 10));
  });

  test_(102, 'Range slider initial value', (steps, chance) => {
    const { wrapper, props } = mountWidthSlider(steps, chance);
    expectFieldValue(steps, wrapper, 'input[type="range"]', String(props.value / 10));
  });

  test_(103, 'Text field event', async (steps, chance) => {
    const { wrapper, props } = mountWidthSlider(steps, chance);
    const xs = chance.nn(chance.integer, 3, 5, { min: props.min, max: props.max });
    for (let [i, x] of xs.entries()) {
      await setFieldValue(steps, wrapper, 'input[type="number"]', String(x / 10), 'change')
      expectEvent(steps, wrapper, 'input', x, i);
    }
  });

  test_(104, 'Range slider event', async (steps, chance) => {
    const { wrapper, props } = mountWidthSlider(steps, chance);
    const xs = chance.nn(chance.integer, 3, 5, { min: props.min, max: props.max });
    for (let [i, x] of xs.entries()) {
  ...
  test_(105, 'Bounds', async (steps, chance) => {
    const { wrapper, props } = mountWidthSlider(steps, chance);
    let xs = [];
    ...
    for (let [i, [x, z]] of xs.entries()) {
      await setFieldValue(steps, wrapper, 'input[type="number"]', String(x / 10), 'change')
      expectEvent(steps, wrapper, 'input', z, i);
    }
  });

  test_(106, 'Decimal truncation', async (steps, chance) => {
    const { wrapper, props } = mountWidthSlider(steps, chance);
    ...
    for (let [i, y] of ys.entries()) {
      ...
      expectEvent(steps, wrapper, 'input', Math.trunc(y * 10), i);
    }
  });
